...
use App\Http\Models\report_transaksi2_mv;
...
class ListTransaksiController {
    ...
    public function getListTransaksi(Request $request){
        ...
        // Total Data
        $totaldata = report_transaksi2_mv::select('mbr_code')
        ->whereIn($columns[11], array('Active', 'Gagal'))
        ...
        if($custom_search == 'true'){
            if(empty($request->input('search.value'))){
                $data_search = report_transaksi2_mv::select(
                    $columns[0],
                    $columns[1],
                    $columns[2],
                    $columns[3],
                ...
                if($start_date && $end_date){
                    ...
                    $data_search = $data_search->where(function($query) use ($columns)
                    {
                        $query->orWhereRaw("tgl_trx > current_date - interval '2' day");
                    });
                }
            ...
            $data = report_transaksi2_mv::select(
                            $columns[0],
                            $columns[1],
                            $columns[2],
                            $columns[3],
    ...
    function updateStatusRefund(Request $request){
        ...
        //Cek Record
        $member = report_transaksi2_mv::where('transaksi_status', 'Active')
        ...
        DB::commit();
        $this->helper->logAction(14, $acode, 'Refund');
        return json_encode([
            "respStatus"    => TRUE,
            "statusTRX"     => "Resend",
